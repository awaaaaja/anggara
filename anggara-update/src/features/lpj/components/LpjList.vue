...
import { useLpjs } from "@/features/lpj/composables/useLpj";
...
const { data, isLoading, isError, error, refetch } = useLpjs(filters);
...
const statusOptions = Object.keys(lpjStatusLabel);

function goDetail(id) {
  router.push(`/${auth.role}/lpj/${id}`);
}
</script>
        ...
        <Table>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3">
                <Badge :variant="lpjStatusVariant[l.status]">{{ lpjStatusLabel[l.status] }}</Badge>
              </td>
              <td class="px-4 py-3 text-right text-text">
                {{ formatRupiah(l.total_realisasi) }}
              </td>
