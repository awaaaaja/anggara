...
type MpmGuard = { ok: false; error: string } | { ok: true; profile: NonNullable<Awaited<ReturnType<typeof getCurrentProfile>>> };

async function guardMpm(): Promise<MpmGuard> {
  const profile = await getCurrentProfile();
  if (!profile) return { ok: false, error: "Sesi tidak valid. Silakan login ulang." };
  if (profile.role !== "mpm") return { ok: false, error: "Anda tidak berwenang melakukan aksi ini." };
  return { ok: true, profile };
}

export async function getMpmSummary() {
  const startOfMonth = new Date();
  startOfMonth.setDate(1);
  startOfMonth.setHours(0, 0, 0, 0);

  const [totalBulanIni] = await db
  ...
  const [totalProposal] = await db.select({ total: count() }).from(proposals);
  const [ormawaAktif] = await db
    .select({ total: count() })
    .from(ormawa)
    .where(eq(ormawa.status, "aktif"));

  const perOrmawa = await db
    .select({
...
export async function listOrmawaAdmin(opts: { page?: number; perPage?: number } = {}) {
  const perPage = opts.perPage ?? 20;
  const page = Math.max(1, opts.page ?? 1);
  const offset = (page - 1) * perPage;

  const [{ total }] = await db.select({ total: count() }).from(ormawa);
  const rows = await db
    .select({
      id: ormawa.id,
      nama: ormawa.nama,
...
export async function createOrmawaAction(input: z.infer<typeof createOrmawaSchema>): Promise<ActionResult> {
  ...
  if (authError || !created.user) {
    return { error: `Gagal membuat akun login: ${authError?.message ?? "kesalahan tidak diketahui"}` };
  }
...
export async function toggleOrmawaStatusAction(ormawaId: string): Promise<ActionResult> {
  const guard = await guardMpm();
  if (!guard.ok) return { error: guard.error };
...
export async function listLpjGallery(opts: { page?: number; perPage?: number } = {}) {
  const perPage = opts.perPage ?? 6;
  const page = Math.max(1, opts.page ?? 1);
  const offset = (page - 1) * perPage;
  const galleryWhere = inArray(proposals.status, ["lpj_direview", "selesai"]);

  const [{ total }] = await db.select({ total: count() }).from(lpj).innerJoin(proposals, eq(lpj.proposal_id, proposals.id)).where(galleryWhere);
  ...
  const docs = lpjIds.length
    ? await db
        .select({ lpjId: dokumentasiKegiatan.lpj_id, fileUrl: dokumentasiKegiatan.file_url, fileType: dokumentasiKegiatan.file_type, caption: dokumentasiKegiatan.caption })
...
export async function listActivityLogs(opts: { actorRole?: string; action?: string; page: number; perPage?: number }) {
  const perPage = opts.perPage ?? 20;
  const offset = (opts.page - 1) * perPage;
  const conditions = [
    opts.actorRole && opts.actorRole !== "semua" ? eq(activityLogs.actor_role, opts.actorRole) : undefined,
...
async function getProposalForReviewMpm(proposalId: string) {
  const data = await getProposalForReview(proposalId);
  if (!data) return null;
  const [{ total }] = await db.select({ total: count() }).from(proposalRevisions).where(eq(proposalRevisions.proposal_id, proposalId));
