...
import { db, queryAs } from "@/lib/db/client";
import {
  anggaran,
  dokumentasiKegiatan,
  lpj,
...
export async function listArsipProposals(userId?: string) {
...
export async function listArsipLpj(userId?: string) {
  return queryAs(userId, async (q) => {
    return q
      .select({
...
export async function getLkpkaSummary(userId?: string) {
  return queryAs(userId, async (q) => {
...
export async function listOrmawaOptions(userId?: string) {
  return queryAs(userId, async (q) => {
    return q.select({ id: ormawa.id, nama: ormawa.nama }).from(ormawa).orderBy(asc(ormawa.nama));
  });
}
...
export async function listProposalsForReview(
  ...
  return queryAs(userId, async (q) => {
    ...
    const sampaiDate =
      opts.sampai && opts.sampai !== "semua" ? new Date(opts.sampai + "T23:59:59") : null;
    const conditions = [
      opts.status && opts.status !== "semua"
        ? eq(proposals.status, opts.status as StatusProposal)
        : undefined,
      opts.ormawaId && opts.ormawaId !== "semua"
    ...
    const sort =
      opts.status === "diajukan" ? asc(proposals.created_at) : desc(proposals.created_at);
...
export async function getProposalForReview(proposalId: string, userId?: string) {
  return queryAs(userId, async (q) => {
    const [proposal] = await q
      .select({
        id: proposals.id,
        judul_kegiatan: proposals.judul_kegiatan,
        divisi_pengaju: proposals.divisi_pengaju,
...
export async function listLpjTracking(
  ...
  return queryAs(userId, async (q) => {
    ...
    const [{ total }] = await q.select({ total: count() }).from(proposals).where(trackingWhere);
...
export async function getLpjForReview(proposalId: string, userId?: string) {
  return queryAs(userId, async (q) => {
    const [proposal] = await q
      .select({
        id: proposals.id,
