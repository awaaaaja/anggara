...
async function guardOrmawa(): Promise<OrmawaGuard> {
  ...
  return {
    ok: true,
    profile: profile as OrmawaGuard extends { ok: true } ? OrmawaGuard["profile"] : never,
  };
}

async function requireOrmawaAktif(ormawaId: string, userId?: string): Promise<string | null> {
...
export async function getOrmawaTahunOptions(ormawaId: string, userId?: string) {
  return queryAs(userId, async (q) => {
    const rows = await q
...
export async function listProposalsOrmawa(
  opts: {
    ormawaId: string;
    status?: string;
    tahun?: string;
  ...
  return queryAs(userId, async (q) => {
    ...
    const conditions = [
      ...
      opts.tahun
        ? and(
            gte(proposals.created_at, new Date(`${opts.tahun}-01-01T00:00:00Z`)),
            lt(proposals.created_at, new Date(`${Number(opts.tahun) + 1}-01-01T00:00:00Z`)),
          )
        : undefined,
    ].filter(Boolean) as Parameters<typeof and>[0][];

    const where = and(...conditions);
    const [{ total }] = await q.select({ total: count() }).from(proposals).where(where);
    const rows = await q
      .select({
        id: proposals.id,
        judul_kegiatan: proposals.judul_kegiatan,
...
export async function getProposalForOrmawa(proposalId: string, ormawaId: string, userId?: string) {
  return queryAs(userId, async (q) => {
    const [own] = await q
      ...
      .limit(1);
    if (!own) return null;
    return getProposalForReview(proposalId, userId);
  });
}

export async function listLpjOrmawa(ormawaId: string, userId?: string) {
  return queryAs(userId, async (q) => {
    return q
      .select({
...
export async function getProposalTimeline(proposalId: string, ormawaId: string, userId?: string) {
  return queryAs(userId, async (q) => {
    const [own] = await q
      .select({ id: proposals.id })
      .from(proposals)
      ...
      .limit(1);
    if (!own) return [];
    const lpjIds = await q.select({ id: lpj.id }).from(lpj).where(eq(lpj.proposal_id, proposalId));
    const logs = await q
      .select({
      ...
      .where(
        or(
          ...
          and(
            eq(activityLogs.target_table, "lpj"),
            inArray(
...
export async function createProposalAction(formData: FormData): Promise<ActionResult> {
  ...
  const nonaktif = await requireOrmawaAktif(profile.ormawa_id, profile.id);
...
export async function resubmitProposalAction(formData: FormData): Promise<ActionResult> {
  ...
  const res = await dbAsUser(profile.id, async (tx): Promise<ActionResult> => {
...
export async function submitLpjAction(formData: FormData): Promise<ActionResult> {
  ...
  const res = await dbAsUser(profile.id, async (tx): Promise<ActionResult> => {
    ...
    await tx.update(proposals).set({ status: "lpj_direview" }).where(eq(proposals.id, proposal.id));
    await logActivity(
      {
        actorId: profile.id,
        actorRole: "ormawa",
